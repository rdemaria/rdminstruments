...
def a2c(a):
  """angle to complex"""
  return exp(1j*pi*a/180.)

def c2a(c):
  """complex to angle"""
  return angle(c)/pi*180.

def c2ua(c):
  return unwrap(c2a(c),discont=180.)

def gdelay(f,c):
  df=f[1]-f[0]
  return r_[0.,diff(unwrap(angle(c)))/2/pi/df]
...
def gauss(t,mu,sigma):
  return 1./sigma/sqrt(2*pi)*exp( -.5*((t-mu)/sigma)**2)

def rect(t,mu,sigma):
  return 1.*(abs(t-mu)<sigma)


def phi(t):
  return .5*(1+erf(t/sqrt(2)))

def delay(f,dt):
  """Return a delay in power of frequency
     2j*pi*dt[0]+2j*pi*dt[1]*f+...
  """
  if not hasattr(dt,'__len__'): dt=[dt]
  a=zeros(len(f))
  nf=1
  for i in dt:
    a+=i*nf
...
def _time(n,fs):
  fs=float(fs)
  return arange(0.,n)/fs

def _freq(n,fs):
  fs=float(fs)
#  return arange(0,fs/2+fs/n,fs/n)
  return arange(0.,n/2+1)*fs/n
...
def ff(n):
  return arange(0,1,1./n)

def rff(n):
  return arange(0,.5+.5/n,1./n)
...
def expcouplertf(f,l=0.40,a=log(0.03/0.002),v=c):
  ...
  s=0*t
  tt=t<2*l/v
  s[tt]=.5*exp(-v*t[tt]*a / (2*l))
  s=s/sum(s)
  tf=2j*pi*f*rfft(s);tf[0]=1E12
  return tf
...
def edgedetect(v,ma,tr):
  w=convolve(abs(v),1./ma*ones(ma),mode='same')
  ww=w.copy()
  w[ww>tr]=1; w[ww<=tr]=0
#  w=r_[w[ma/2:],w[:ma/2]]
  w=diff(w)
...
def edgeget(v,w,wl,bn):
  start=where(w==1)[0]; stop=where(w==-1)[0]
  w=c_[start,stop]
  c=(w[bn,0]+w[bn,1])/2
...
def movavg2(x,n=100):
  ...
  return convolve(ones(float(n))/n,x,mode='same')

def movavg3(x,n=100,reverse=False):
  """moving average with rect, fixside"""
  x=convolve(ones(float(n))/n,x,mode='same')
  x[:n/2]=x[n/2]
  return x
...
def h5print(x,pk=''):
  ...
  if  hasattr(x,'shape') and hasattr(x,'dtype'):
    print '%s[%s]' % (x.dtype.str,','.join(map(str,x.shape))),
  elif type(x) in [int,float,long]:
    print x,
  elif type(x) in [str]:
    if len(x)>20:
      print '%s..%s' % (x[:8],x[-8:]),
